...
public final class TileEntityModernParityRenderer {
    ...
    /** Renders the three Shelf inventory stacks in their front-facing compartments. */
    public static final class Shelf extends TileEntitySpecialRenderer {
        private final EntityItem[] renderItems = new EntityItem[3];

        @Override public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float partialTicks) {
            if (!(tile instanceof ModernMapParityBlocks.ParityShelfTileEntity) || !tile.hasWorldObj()) return;
            ModernMapParityBlocks.ParityShelfTileEntity shelf = (ModernMapParityBlocks.ParityShelfTileEntity) tile;
            int facing = tile.getBlockMetadata() & 3;
            for (int slot = 0; slot < 3; slot++) {
                ...
                EntityItem entity = renderItems[slot];
                entity.setWorld(tile.getWorldObj());
                ItemStack renderStack = stack.copy(); renderStack.stackSize = 1;
                ...
                float scale = block3d ? 0.42F : 0.62F;
                GL11.glScalef(scale, scale, scale);
                RenderItem.renderInFrame = true;
                RenderManager.instance.renderEntityWithPosYaw(entity, 0.0D, 0.0D, 0.0D, 0.0F, 0.0F);
                RenderItem.renderInFrame = false;
        ...
        private static double[] rotate(double px, double pz, int facing) {
            double x = px - 0.5D, z = pz - 0.5D;
            switch (facing & 3) {
                case 1: return new double[]{0.5D - z, 0.5D + x};
                case 2: return new double[]{0.5D - x, 0.5D - z};
                case 3: return new double[]{0.5D + z, 0.5D - x};
